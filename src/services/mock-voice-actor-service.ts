...
export class MockVoiceActorService {
  ...
  async searchVoiceActors(filters: VoiceActorSearchFilters = {}): Promise<VoiceActor[]> {
    ...
    if (filters.experience?.length) {
      results = results.filter(actor => 
        filters.experience!.includes(actor.experience)
      )
    }
  ...
  async createVoiceSample(sample: Omit<VoiceSample, 'id' | 'createdAt' | 'updatedAt'>): Promise<VoiceSample> {
    await delay()
  ...
  async submitToCasting(sessionId: string, submission: Omit<CastingSubmission, 'id' | 'createdAt' | 'updatedAt'>): Promise<CastingSubmission> {
    await delay()
  ...
  // Statistics
  async getVoiceActorStats(actorId: string): Promise<VoiceActorStats> {
    await delay()
  ...
  // Activity Feed
  async getRecentActivity(actorId: string, limit: number = 10): Promise<ActivityItem[]> {
    await delay()
    return mockData.voiceActorStats.recentActivity.slice(0, limit)
  }

  // File Upload Simulation
  async uploadFile(file: File, type: 'sample' | 'recording' | 'reference'): Promise<{ url: string; id: string }> {
  ...
  async processAudio(audioUrl: string): Promise<{ waveformUrl: string; duration: number }> {
