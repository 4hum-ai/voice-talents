<template>
  <div class="relative mx-auto flex max-w-[32rem] flex-col justify-center px-4 py-12">
    <div class="pointer-events-none absolute inset-x-0 top-6 mx-auto h-32 w-32 rounded-full bg-red-500/10 blur-3xl"></div>

    <div class="relative rounded-2xl border border-zinc-800 bg-zinc-900/60 p-6 shadow-2xl backdrop-blur">
      <button
        class="absolute right-4 top-4 inline-flex h-9 w-9 items-center justify-center rounded-lg border border-zinc-800 bg-zinc-900 text-zinc-300 hover:text-white hover:border-zinc-700"
        type="button"
        @click="toggleTheme()"
      ...
      <div class="mb-6 flex items-center gap-3">
        ...
        <div>
          <h1 class="text-xl font-semibold leading-6">Sign in to Admin</h1>
          <p class="text-sm text-zinc-400">Access your workspace and tools</p>
        </div>
      </div>
      ...
      <form @submit.prevent="handleEmailLogin" class="space-y-4">
        <div class="relative">
          ...
          <input v-model="email" type="email" placeholder="Email" class="w-full rounded-lg border border-zinc-700 bg-zinc-900 pl-10 pr-3 py-2.5 text-zinc-100 placeholder-zinc-500 focus:border-red-500 focus:outline-none focus:ring-1 focus:ring-red-500" required />
        </div>
        ...
        <div class="relative">
          ...
          <input :type="showPassword ? 'text' : 'password'" v-model="password" placeholder="Password" class="w-full rounded-lg border border-zinc-700 bg-zinc-900 pl-10 pr-10 py-2.5 text-zinc-100 placeholder-zinc-500 focus:border-red-500 focus:outline-none focus:ring-1 focus:ring-red-500" required />
          <button type="button" class="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-400 hover:text-zinc-200" @click="showPassword = !showPassword" :aria-label="showPassword ? 'Hide password' : 'Show password'">
        ...
        <div class="flex items-center justify-between text-sm">
          <label class="inline-flex items-center gap-2 text-zinc-400">
            <input type="checkbox" class="h-4 w-4 rounded border-zinc-700 bg-zinc-900 text-red-600 focus:ring-red-600" />
            <span>Remember me</span>
          </label>
          <a href="#" class="text-zinc-400 hover:text-zinc-200">Forgot password?</a>
        </div>
        ...
        <button :disabled="isLoading" class="btn-primary w-full h-11 disabled:opacity-60 disabled:cursor-not-allowed">
          ...
          <span v-else class="inline-flex items-center justify-center gap-2">
            <span class="inline-block h-4 w-4 animate-spin rounded-full border-2 border-zinc-300 border-t-transparent"></span>
            Signing in...
          </span>
        </button>
      </form>
      ...
      <div class="relative my-6">
        ...
        <span class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-zinc-900/60 px-2 text-xs uppercase tracking-wide text-zinc-400">Or</span>
      </div>
