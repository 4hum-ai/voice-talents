<template>
  <div class="relative mx-auto flex max-w-[32rem] flex-col justify-center px-4 py-12">
    <div class="pointer-events-none absolute inset-x-0 top-6 mx-auto h-32 w-32 rounded-full bg-primary-500/10 blur-3xl" />
    ...
    <div class="relative rounded-2xl border border-gray-200 dark:border-gray-800 bg-white/80 dark:bg-gray-900/60 p-6 shadow-2xl backdrop-blur">
      ...
      >
        <div class="relative">
          <span class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500">
            <svg
              ...
              fill="currentColor"
              class="h-5 w-5"
            ><path d="M1.5 8.67v6.66A3.17 3.17 0 0 0 4.67 18.5h14.66a3.17 3.17 0 0 0 3.17-3.17V8.67a3.16 3.16 0 0 0-.53-1.75l-8.32 5.2a2.25 2.25 0 0 1-2.39 0L2.03 6.92a3.16 3.16 0 0 0-.53 1.75Z" /><path d="M22.5 6.75a3.17 3.17 0 0 0-3.17-3.17H4.67A3.17 3.17 0 0 0 1.5 6.75l9.01 5.63a.75.75 0 0 0 .78 0l9.21-5.63Z" /></svg>
          </span>
          <input
            v-model="email"
            type="email"
            placeholder="Email"
            class="w-full rounded-lg border border-gray-300 bg-white pl-10 pr-3 py-2.5 text-gray-900 placeholder-gray-400 focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 dark:placeholder-gray-500 dark:focus:border-primary-600 dark:focus:ring-primary-600"
            required
          >
        </div>
        ...
        <div class="relative">
          <span class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500">
            <svg
              ...
              fill="currentColor"
              class="h-5 w-5"
            ><path d="M12 1.5a5.25 5.25 0 0 0-5.25 5.25v3h-.75A2.25 2.25 0 0 0 3.75 12v7.5A2.25 2.25 0 0 0 6 21.75h12A2.25 2.25 0 0 0 20.25 19.5V12A2.25 2.25 0 0 0 18 9.75h-.75v-3A5.25 5.25 0 0 0 12 1.5Zm-3.75 8.25v-3a3.75 3.75 0 1 1 7.5 0v3h-7.5Z" /></svg>
          </span>
          <input
            v-model="password"
            :type="showPassword ? 'text' : 'password'"
            placeholder="Password"
            class="w-full rounded-lg border border-gray-300 bg-white pl-10 pr-10 py-2.5 text-gray-900 placeholder-gray-400 focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 dark:placeholder-gray-500 dark:focus:border-primary-600 dark:focus:ring-primary-600"
            required
          >
          <button
            type="button"
          ...
          >
            <svg
              ...
              fill="currentColor"
              class="h-5 w-5"
            ><path d="M12 5C7 5 2.73 8.11 1 12c1.73 3.89 6 7 11 7s9.27-3.11 11-7c-1.73-3.89-6-7-11-7Zm0 12a5 5 0 1 1 0-10 5 5 0 0 1 0 10Z" /><path d="M12 9a3 3 0 1 0 0 6 3 3 0 0 0 0-6Z" /></svg>
            <svg
              v-else
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
        ...
        <div class="flex items-center justify-between text-sm">
          <label class="inline-flex items-center gap-2 text-gray-600 dark:text-gray-400">
            <input
              v-model="remember"
              type="checkbox"
              class="h-4 w-4 rounded border-gray-300 bg-white text-primary-600 focus:ring-primary-600 dark:border-gray-700 dark:bg-gray-900"
            >
        ...
        <button
          :disabled="isLoading"
          class="btn-primary w-full h-11 disabled:opacity-60 disabled:cursor-not-allowed"
        >
          ...
          >
            <span class="inline-block h-4 w-4 animate-spin rounded-full border-2 border-gray-300 border-t-transparent dark:border-gray-500" />
            Signing in...
          </span>
        </button>
      </form>
      ...
      <p
        v-if="error"
        class="mt-4 text-sm text-error-600 dark:text-error-300"
      >
        {{ error }}
      </p>
    </div>
...
const handleEmailLogin = async () => {
  try {
    ...
    try { (await import('@/composables/useAuth')).useAuth().setPersistenceMode(remember.value ? 'local' : 'session') } catch { void 0 }
