...
interface KanbanStatusChangePayload {
  item: ItemData
  from: string
  to: string
}

interface FiltersChangePayload {
  preset?: string
  from?: string
  to?: string
...
const currentView = computed<ViewType>(() => {
  ...
  if (!v) v = 'list'
  return v === 'gallery' || v === 'kanban' || v === 'calendar' ? (v as ViewType) : 'list'
})
  ...
  for (const f of filters) {
    ...
    if (eq !== undefined) {
      ...
      if (filterConfig.options && Array.isArray(filterConfig.options)) {
        const found = filterConfig.options.find((o: FilterOption) => String(o.value) === String(eq))
...
async function loadUiConfig() {
  try {
    ...
    const errorMessage = e instanceof Error ? e.message : 'Failed to load UI configuration'
...
async function load(page = 1) {
  ...
  try {
    ...
    currentAbort?.abort()
    currentAbort = new AbortController()
    const res: PaginatedResult<ItemData> = await api.list(
      resource.value,
      params,
      currentAbort.signal,
    )
...
function onAction(action: string, payload?: ItemData) {
  if (action === 'create') {
    api
...
async function onKanbanStatusChange(payload: KanbanStatusChangePayload) {
  try {
...
async function onLoadMore() {
  ...
  try {
    ...
    const errorMessage = e instanceof Error ? e.message : 'Failed to load more'
    error.value = errorMessage
  } finally {
    loading.value = false
  }
...
function onFiltersChange(payload: FiltersChangePayload) {
