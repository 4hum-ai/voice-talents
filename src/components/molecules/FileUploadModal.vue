<template>
  ...
  >
    <div class="h-full w-full bg-background">
      <!-- Header -->
      <div class="flex items-center justify-between border-b px-5 py-3 border-border">
        <div class="flex items-center gap-3">
          <h2 class="text-base font-semibold text-foreground">Upload Media</h2>
          <p class="hidden text-xs text-gray-500 sm:block dark:text-gray-400">
            Select files on the left, set details on the right
          </p>
        </div>
        <button
          class="flex items-center gap-2 rounded-md border border-border px-3 py-1.5 text-sm text-foreground hover:bg-muted"
          aria-label="Close"
          @click="$emit('close')"
        >
      ...
      <div class="grid h-[calc(100%-3rem)] grid-cols-1 sm:grid-cols-5">
        <div class="col-span-1 p-4 sm:col-span-3 sm:border-r sm:p-6 border-border">
          <div
            v-if="!files.length"
          ...
          >
            <div class="text-center">
              <CloudUploadIcon
                ...
                aria-hidden="true"
              />
              <div class="mt-3 text-sm text-muted-foreground">Drag & drop files</div>
              <div class="mt-1 text-xs text-muted-foreground">or</div>
              <button
          ...
          <div v-else class="overflow-y-auto sm:h-full">
            ...
            <div v-if="previewMode === 'single' && previews.length === 1" class="space-y-3">
              <div class="overflow-hidden rounded-lg border border-border bg-muted/5">
                <Image
                ...
                <audio
                  ...
                  class="w-full"
                />
                <div v-else class="p-8 text-sm text-muted-foreground">
        ...
        <div class="col-span-1 overflow-y-auto p-4 sm:col-span-2 sm:p-6">
          <div class="space-y-3">
            ...
            >
              ...
              <input
                ...
                class="w-full rounded-md border px-2 py-1.5 text-sm border-border bg-input text-foreground"
              />
            ...
            <div v-if="previews.length === 1 && previews[0].duration !== undefined">
              ...
              <input
                ...
                readonly
                disabled
                class="w-full cursor-not-allowed rounded-md border bg-muted px-2 py-1.5 text-sm text-foreground"
              />
            </div>
          </div>
          <div
            class="sticky bottom-0 mt-4 flex justify-end gap-2 border-t bg-card/80 px-4 py-3 backdrop-blur border-border"
          >
          ...
          <div v-if="uploading" class="sticky right-0 bottom-0 left-0 h-1 bg-muted">
