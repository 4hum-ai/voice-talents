...
const props = defineProps<{ items: MenuItem[]; size?: 'sm' | 'md' | 'lg' | 'xl' }>()
...
const itemPadding = computed(() => {
  const map: Record<string, string> = { sm: 'px-3 py-1.5', md: 'px-4 py-2', lg: 'px-4 py-2.5', xl: 'px-5 py-3' }
...
const stopWatch = watch([buttonEl, itemsEl], ([btn, items]) => {
  cleanup.value?.()
  if (btn && items) {
