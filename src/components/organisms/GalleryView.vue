<template>
  <div class="space-y-6">
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 lg:gap-6">
      <div v-for="item in data" :key="item.id" class="bg-[#0f1116] rounded-lg border border-[#1e2130] overflow-hidden cursor-pointer" @click="handleItemClick(item)">
        <div class="aspect-square bg-[#111318] flex items-center justify-center">
          <span class="text-4xl text-gray-500 font-medium">{{ getInitials(item[config.titleField]) }}</span>
        </div>
        <div class="p-4">
          <h3 class="font-medium text-sm mb-1">{{ item[config.titleField] }}</h3>
