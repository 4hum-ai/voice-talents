...
interface Props { data: any[]; config: { columns: any[]; groupByField: string; cardTitleField: string; cardDescriptionField?: string; actions?: any[] }; moduleName?: string; uiConfig?: any; loading?: boolean; activeFilters?: { key: string; label: string }[] }
...
function handleFormSubmit(data: Record<string, any>){ emit('create', data); closeFormSidebar() }
...
function openFilters(){ showFilterSidebar.value = true }
...
function onTimeWindowChange(payload: { preset?: string; from?: string; to?: string }) {
  ...
  emit('filters-change', { preset: selectedPreset.value, from: fromISO.value, to: toISO.value })
}
...
const setColumnRef = (value: string, el: HTMLElement | null) => { columnRefs[value] = el }
...
const handleDropFromDraggable = (id: string) => {
  ...
  if (inst) { inst.x.value = 0; inst.y.value = 0 }
  if (cx == null || cy == null) { draggingItemId.value = null; return }
  ...
  if (!targetColumnValue || from === to) { draggingItemId.value = null; return }
...
function applyFilters(){
  const payload: Record<string, any> = { preset: selectedPreset.value, from: fromISO.value, to: toISO.value }
