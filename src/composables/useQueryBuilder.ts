...
export function useQueryBuilder(options: {
  ...
  const dateField = computed(
    () => options.uiConfig()?.views?.calendar?.dateField || "createdAt",
  );
  ...
  const queryState = computed<QueryState>(() => {
    ...
    const searchField =
      (q.searchFields as string) || (q.searchField as string) || undefined;
    ...
    if (range && (range.from || range.to)) {
      ...
      if (range.from && range.to)
        ...
        params.filters[dateField.value] = { $gte: range.from };
  ...
  function setTimeWindow(payload: {
    preset?: string;
    from?: string;
    to?: string;
  }) {
