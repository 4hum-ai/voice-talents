...
export interface GetUiConfigOptions { force?: boolean; signal?: AbortSignal }
...
const state = reactive<UiConfigState>({ configs: {}, initialized: false, loading: false, error: null })

const LOCAL_STORAGE_KEY = 'admin-ui:configs'

const configFromLocal = (() => {
  const env: any = (import.meta as any).env || {}
  return env?.VITE_ADMIN_UI_CONFIG_FROM_LOCAL === 'true' || env?.VITE_ADMIN_UI_CONFIG_FROM_LOCAL === true
})()
const localPath = (() => {
  const env: any = (import.meta as any).env || {}
...
type AdminModuleInfo = { name: string; displayName: string; description?: string; icon?: string; path: string }
...
export function useUiConfig() {
  ...
  const saveToCache = () => {
    try { localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(state.configs)) } catch { /* ignore */ }
  }
  ...
  const fetchconfigFromLocal = async (moduleName: string, signal?: AbortSignal): Promise<UiConfig | null> => {
    const res = await fetch(`/ui-configs/${moduleName}.json`, { cache: 'no-store', signal })
  ...
  const listModules = async (opts: { force?: boolean; signal?: AbortSignal } = {}): Promise<AdminModuleInfo[]> => {
    if (!opts.force && cachedModules) return cachedModules
    if (!opts.force && modulesPromise) return modulesPromise
    modulesPromise = (async () => {
      // Try local index first
      try {
        const res = await fetch(localPath, { cache: 'no-store', signal: opts.signal })
        if (res.ok) {
          const contentType = res.headers.get('content-type') || ''
      ...
      // Fallback to backend
      try {
        const res = await api.request(`/api/admin-ui/modules`, { method: 'GET', signal: opts.signal })
  ...
  const get = async (moduleName: string, opts: GetUiConfigOptions = {}): Promise<UiConfig | null> => {
    if (!opts.force) {
    ...
    // Fallback to backend
    try {
      const res = await api.request(`/api/admin-ui/modules/${moduleName}/config`, { method: 'GET', signal: opts.signal })
      if (!res.ok) return null
      const contentType = res.headers.get('content-type') || ''
      if (!/application\/json/.test(contentType)) return null
      const payload = await res.json().catch(() => null)
